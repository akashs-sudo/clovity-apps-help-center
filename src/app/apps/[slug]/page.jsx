        ...
        <div>
          ...
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14 sm:py-18">
              <div className="flex flex-col sm:flex-row items-start gap-7">
                <div className="w-24 h-24 rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-xl shrink-0">
                  <Image src={app.icon} alt={app.shortName} width={96} height={96} unoptimized className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-xs text-gray-400 flex items-center gap-1.5">
                      ...
                      {app.rating}.0 · {app.installs?.toLocaleString()}+ installs
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
            <div className="flex items-center justify-between mb-7">
              ...
              <span className="text-xs text-gray-400">{docs.categories.length} categories · {docs.categories.reduce((n, c) => n + c.articles.length, 0)} articles</span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      >
                        <svg className="w-5 h-5" fill="none" stroke={color.bar} viewBox="0 0 24 24">
                      ...
                      <p className="text-xs font-medium mb-3" style={{ color: color.bar }}>
                        {cat.articles.length} {cat.articles.length === 1 ? "article" : "articles"}
                      </p>

                      <ul className="space-y-1.5">
                        {cat.articles.slice(0, 3).map((article) => (
                          <li key={article.slug} className="text-xs text-gray-500 flex items-center gap-2 leading-snug">
                            <span className="w-1 h-1 rounded-full shrink-0" style={{ backgroundColor: color.bar }} />
                            {article.title}
                          </li>
                        ))}
                        {cat.articles.length > 3 && (
                          <li className="text-xs pl-3" style={{ color: color.bar }}>
                            +{cat.articles.length - 3} more
                          </li>
                        )}
                      </ul>

                      <div className="mt-4 flex items-center gap-1 text-xs font-medium group-hover:gap-2 transition-all" style={{ color: color.bar }}>
                        View all
